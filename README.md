...
본부급 계정만 사용할 수 있는 대량 `deposit` 기능입니다. 엑셀 필수 컬럼은 아래 5개입니다. `지급 수량` 컬럼은 앞뒤 공백, `지급수량`, `지급 수량(수식 변환)` 같은 변형도 허용합니다.
...
검증 기준은 숫자만 남긴 연락처와 지갑주소 전체 일치입니다. 회원명은 검증 조건에서 제외하고 엑셀 회원명/DB 회원명을 비교용으로 표시합니다. 지급 수량은 쉼표/공백을 제거하고 소수점 8자리 초과분은 반올림 없이 버림 처리합니다. 엑셀 내 동일 연락처·지갑주소 중복, 금액 오류, 회원/주소 불일치, 중복 회원 매칭, 이미 완료된 동일 파일 SHA-256 해시가 있으면 전체 지급이 반려됩니다. 실행 시에는 deposit 처리 사유 프리셋을 선택하거나 직접 입력하고, `PAY_N_ROWS` 확인 문구를 입력해야 합니다. 성공 후 `transactions.transaction_type = deposit` 기록, 처리 사유, 관리자 감사 로그가 생성됩니다.

회원 상세와 회원 목록의 `mb_hp`는 `000-0000-0000` 형식으로 표시되며 복사 시에도 하이픈 포함 값이 사용됩니다. 회원 검색은 하이픈/공백/점 유무와 무관하게 전화번호를 비교합니다.

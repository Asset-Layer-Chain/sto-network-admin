-- STO Network Product Admin - 팀장급 회원 검색 전용 접근
-- team_lead는 전체 회원 목록을 열람할 수 없고, 정확한 회원 식별정보 검색 결과만 조회한다.

create or replace function private.team_lead_member_search_match(
  p_user_id uuid,
  p_member_no bigint,
  p_member_id text,
  p_member_name text,
...
as $$
  select
    lower(coalesce(p_member_id, '')) = lower(trim(coalesce(p_search, '')))
    or lower(coalesce(p_member_name, '')) = lower(trim(coalesce(p_search, '')))
    or lower(coalesce(p_member_email, '')) = lower(trim(coalesce(p_search, '')))
    or regexp_replace(coalesce(p_member_phone, ''), '[^0-9]', '', 'g') =
       regexp_replace(trim(coalesce(p_search, '')), '[^0-9]', '', 'g')
       and regexp_replace(trim(coalesce(p_search, '')), '[^0-9]', '', 'g') <> ''
    or p_user_id::text = trim(coalesce(p_search, ''))
    or p_member_no::text = trim(coalesce(p_search, ''));
$$;

revoke all on function private.team_lead_member_search_match(uuid, bigint, text, text, text, text, text)

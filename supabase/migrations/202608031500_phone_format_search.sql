-- STO Network Product Admin - 전화번호 표시/검색 포맷 보정
-- 회원 상세/목록 화면은 하이픈 형식으로 표시하고, 검색 RPC는 하이픈/공백/점 유무와 무관하게 전화번호를 비교한다.
         ...
         when v_search_only then
           ...
           or exists (
             select 1
              ...
              (v_search_phone <> '' and regexp_replace(coalesce(m.mb_hp, ''), '[^0-9]', '', 'g') ilike '%' || v_search_phone || '%') or
